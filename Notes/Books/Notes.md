...
- leader: 
  ...
  - Note: if two nodes both think they are leader, called split brain. Dangerous since both go out of sync, some systems have auto shutdown in this case
...
- Good way to ensure replication lag doesn't cause inconsistent states but becomes extremely expensive and unfit for many distributed systems
...
Partitioning Methods:
  - By Key Range:
    ...
    - 🔴Even if keyspace is evenly distributed data might not be (for e.g. more name begins with certain letters of alphabet)
    - 🔴Hotspots can occur temporarily which block concurrency for e.g. if key is date then all writes will go to same node each day
  ...
  - Note: All of these are primary key based partitioning since they map record to data. Secondary key partitioning is far more complex since no longer 1:1. Avoid if possible
  - Secondary Key Based:
    - Index based:
      - Instead of searching through entire DB we maintain an index on each node (called local index) or using a larger index which links to entry on each node (global index/term partitioning)
      ...
      - 🔴If we add a new secondary key later DB has to parse all historical data and find entries to insert into index (called backfilling)
      - 🟢Reads are cheap
...
- Strategies:
  - hash mod N
    - 🔴Never do this
    - %N will split between N easiley but whenever there is a single change in nodes a lot of keys will need moving since hash mod N will change for all
