...
3. ### Hash Table / Hash Map
   ...
   Time Complexity:
    ...
    - Note: Just because the time complexity is identical to array it's still slower because it can't do random
      memory access, Has to calculate hash first then go to value
...
1. ## Recursion
   A function calls itself within its own definition  
   The idea is take one step toward solution and then recursively call itself to further move. Stop once solution is
   reached  
   ...
   first.
    ...
    - Example:
        - The Problem: The Staircase Variations
          ...
          steps at a time. Write a recursive function to calculate the total number of distinct ways you can reach the
          top.
          Constraints
            - $n \ge 0$
        ...
        ```
      Applications:
      - Tree and Graph Traversal: Used for systematically exploring nodes/vertices in data structures like trees
      and 
      graphs.
